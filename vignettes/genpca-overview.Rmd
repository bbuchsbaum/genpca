...
# Scores and loadings follow multivarious conventions
dim(multivarious::scores(fit))    # n × k matrix
dim(multivarious::loadings(fit))  # p × k matrix
```
...
# (e.g., adjacent time points or spatial neighbors)
A <- bandSparse(p, p, k = -1:1, 
                diagonals = list(
                  rep(0.8, p),      # diagonal (self-correlation)
                  rep(-0.2, p-1),   # off-diagonal (negative correlation with neighbors)
                  rep(-0.2, p-1)
                ))

# Ensure positive semi-definiteness
A <- A + 0.5 * Diagonal(p)  # shift spectrum
...
plot(multivarious::loadings(fit)[,1], 
     multivarious::loadings(fit_corr)[,1],
     xlab = "Standard PCA Loading", 
     ylab = "GPCA Loading (with dependencies)",
     main = "How column metrics change loadings")
...
# This would be memory-intensive with eigen but efficient with spectra
fit_large <- genpca(X_sparse, ncomp = 10, method = "spectra")
```
...
pc1_unweighted <- multivarious::loadings(pca_unweighted)[,1]
pc1_weighted <- multivarious::loadings(pca_weighted)[,1]

# The weighted analysis gives different importance to questions
plot(pc1_unweighted, pc1_weighted,
     xlab = "Unweighted PC1 Loadings",
     ylab = "Population-Weighted PC1 Loadings",
