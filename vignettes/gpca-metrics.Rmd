...
because of the order in which it was assembled. The checks are relative to
the scale of the matrix, so ordinary floating-point noise never triggers
anything: eigenvalues down to $-\sqrt{\epsilon}\,\max|A_{ii}|$ (about
...
A metric that passes is used exactly as supplied.

Two things are never repaired. A genuinely asymmetric matrix is an error under
