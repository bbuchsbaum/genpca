...
This vignette collects practical recipes for row/column metrics, plus notes on SPD remedies and the experimental `gpca_mle()`.

Why metrics
-----------
Metrics encode weighting and correlation. Row metric `M` changes how observations are compared; column metric `A` changes how variables are compared. Both must be symmetric positive definite (SPD) for GPCA.

Runnable example: heteroscedastic diagonals
-------------------------------------------

The simplest non-trivial metrics are diagonal matrices that down-weight noisy rows or columns:
...
When you suspect structured noise but lack good priors, `gpca_mle()` can learn SPD metrics by alternating GPCA with matrix-normal maximum likelihood:
...
Keep `lambda` non-zero, start with small `ncomp`, and inspect warnings (they mean a metric was repaired).
...
See `vignette("gpca-scale", package = "genpca")` for backend choices, sparse workflows, and covariance-only GPCA.
